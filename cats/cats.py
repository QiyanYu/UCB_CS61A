...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    total, length = 0, len(typed_words)
    if length == 0:
        return 0.0
    for i in range(len(reference_words)):
        if i < length and reference_words[i] == typed_words[i]:
            total += 1
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    if user_word in valid_words:
        return user_word
    ret = min(valid_words, key=lambda w: diff_function(user_word, w, limit))
    return ret if diff_function(user_word, ret, limit) <= limit else user_word
    # END PROBLEM 5
